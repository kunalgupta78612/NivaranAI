...
import { useCurrentCitizen } from '../lib/authApi'

/**
 * PublicRoute component for Login/Register pages:
 * Verifies session using GET /api/auth/me via useCurrentCitizen()
 * - Shows styled loading spinner while checking authentication
 * - If user IS authenticated, redirects to /citizen/dashboard
 * - If user IS NOT authenticated, allows access to Login/Register page
 */
export default function PublicRoute({ children }) {
  const { data, isLoading } = useCurrentCitizen()
    ...
    return (
      <div className="min-h-screen grid place-items-center bg-[#EEF2FF]" style={{
        ...
        >
          ...
          <p className="text-xs text-slate-400 font-semibold mt-1.5">Checking active session...</p>
  ...
  const isAuthenticated = Boolean(data?.citizen || data?.success)

...
import { useCurrentDepartment } from '../lib/departmentAuthApi'

/**
 * DepartmentProtectedRoute component
 * Verifies department session using GET /api/department/me via useCurrentDepartment()
 * - Shows styled loading spinner while checking authentication
 * - Allows access if authenticated as department
 * - Redirects to /login if unauthenticated
 */
export default function DepartmentProtectedRoute({ children }) {
  const location = useLocation()
  const { data, isLoading, isError } = useCurrentDepartment()
    ...
    return (
      <div className="min-h-screen grid place-items-center bg-[#EEF2FF]" style={{
        ...
        >
          ...
          <p className="text-xs text-slate-400 font-semibold mt-1.5">Verifying department session...</p>
  ...
  const isAuthenticated = Boolean(data?.department || data?.success)
